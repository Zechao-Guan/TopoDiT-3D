...
class GaussianDiffusion:
    ...
    def reconstruct(self, x0, t, y, denoise_fn, noise_fn=torch.randn, topo_shape=None, constrain_fn=lambda x, t:x):
        ...
        for k in reversed(range(0,t)):
            ...
            img_t = self.p_sample(denoise_fn=denoise_fn, data=img_t, t=t_, noise_fn=noise_fn, y=y,
                                  clip_denoised=False, return_pred_xstart=False, use_var=True, topo_shape=topo_shape).detach()
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        if args.use_resampler:
            self.model = DiT3D_resampler_models[args.model_type](input_size=args.voxel_size, num_classes=args.num_classes)
        else:
            self.model = DiT3D_models[args.model_type](input_size=args.voxel_size, num_classes=args.num_classes)
...
def get_vae(path, device):
    ...
    #model = vae_models['VQVAE'](in_channels=2, embedding_dim=64, num_embeddings=512, beta=0.25, img_size=64)
...
def generate_eval(model, opt, gpu, outf_syn, evaluator):
    ...
    with torch.no_grad():
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=len(test_dataloader), desc='Generating Samples'):
            ...
            vae = get_vae(opt.vae_path, f'cuda:{gpu}')
            topo_shape = vae.sample(x.shape[0], f'cuda:{gpu}')

            gen = model.gen_samples(x.shape, gpu, new_y_chain(gpu,y.shape[0],opt.num_classes), clip_denoised=False, topo_shape=topo_shape).detach().cpu()
...
def parse_args():
    ...
    parser.add_argument('--use_resampler', default=True)
    ...
    parser.add_argument('--dist_url', type=str, default='tcp://localhost:1234')
