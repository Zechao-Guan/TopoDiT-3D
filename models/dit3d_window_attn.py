...
class DiTBlock(nn.Module):
    ...
    def __init__(self, hidden_size, 
        ...
        """
        if window_size != 0:
            self.linear1 = nn.Linear(hidden_size * window_size * window_size * window_size, hidden_size)
            self.linear2 = nn.Linear(hidden_size, hidden_size * window_size * window_size * window_size)
        """
    ...
    def forward(self, x, c):
        ...
        if self.window_size > 0:
            ...
            x, pad_xyz = window_partition(x, self.window_size) #(-1, window_size, window_size, window_size, C)， #计算填充后的维度
        ...
        #above have a problem, it should be:
        """
        # attention 减少token
        if self.window_size > 0:
            X, Y, Z = x.shape[1], x.shape[2], x.shape[3]
            ...
            x = x.reshape(B, *pad_xyz, -1)        #窗口之后的维度
            _B, _X, _Y, _Z, _D = x.shape

            x = x.reshape(_B, _X*_Y*_Z, _D)
            x = modulate(x, shift_msa, scale_msa) #偏置项
            x = x.reshape(_B, *pad_xyz, -1)

            x = self.linear1(x)                   # D*R*R*R -> D
            x = self.attn(x)                      # 需要修改attention层的input_size, 改成缩小后的token数, input_size // window_size

            x = self.linear2(x)                   # D -> D*R*R*R
            x = x.reshape(-1, window_size, window_size, window_size, C)
        else:
            x = x.reshape(_B, _X*_Y*_Z, -1)
            x = modulate(x, shift_msa, scale_msa) #偏置项
            x = x.reshape(_B, *pad_xyz, -1)
            x = self.attn(x)
