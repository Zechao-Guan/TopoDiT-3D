...
class PerceiverResample(nn.Module):
    def __init__(self, hidden_size, num_heads, num_latents, mlp_ratio=4, depth=4, qkv_bias=True, use_pos=False, pos_embed=None):
        super().__init__()
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.down_resample = PerceiverResample(hidden_size, num_heads, num_latents=num_latents, mlp_ratio=mlp_ratio, depth=resample_depth)
    ...
    def forward(self, x, t, y):
        ...
        # Down Resample
        res = x
        x = self.down_resample(x)

        for block in self.blocks:
            x = block(x, c)

        # Up Resample
        x = self.up_resample(x, self.pos_embed)
        x = x + res
