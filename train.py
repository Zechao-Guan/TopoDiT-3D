from topology_vae.PyTorch_VAE.models import *
...
from datasets.shapenet_data_pi import ShapeNet15kPointClouds

from copy import deepcopy
from collections import OrderedDict

from models.dit3d import DiT3D_models
from models.dit3d_topy import DiT3D_resampler_models
from models.dit3d_window_attn import DiT3D_models_WindAttn

from tensorboardX import SummaryWriter

def get_vae(path, device):
    model = vae_models['VanillaVAE'](in_channels=2, latent_dim=128)
    checkpoint = torch.load(path, map_location=device)

    new_state_dict = {}
    for k, v in checkpoint["state_dict"].items():
        name = k[6:]  # 去掉 'model.' 前缀
...
class GaussianDiffusion:
    ...
    def _extract(a, t, x_shape):
        ...
        For example:
        a: tensor([0.1000, 0.2000, 0.3000, 0.4000, 0.5000])
        t: tensor([0, 2, 4])
        x_shape: (3, 64, 64, 3)
        extracted: tensor([[[[0.1000]]], [[[0.3000]]], [[[0.5000]]]])
        """
        bs, = t.shape
        assert x_shape[0] == bs
    ...
    def p_mean_variance(self, denoise_fn, data, t, y, clip_denoised: bool, return_pred_xstart: bool, topo_shape):
        
        # print('data:', data.shape)
        model_output = denoise_fn(data, t, y, topo_shape=topo_shape)
    ...
    def p_sample(self, denoise_fn, data, t, noise_fn, y, clip_denoised=False, return_pred_xstart=False, topo_shape = None):
        """
        ...
        model_mean, _, model_log_variance, pred_xstart = self.p_mean_variance(denoise_fn, data=data, t=t, y=y, clip_denoised=clip_denoised,
                                                                 return_pred_xstart=True, topo_shape=topo_shape)
    ...
    def p_sample_loop(self, denoise_fn, shape, device, y,
                      noise_fn=torch.randn, clip_denoised=True, keep_running=False, topo_shape=None):
        """
        Generate samples
        ...
        for t in reversed(range(0, self.num_timesteps if not keep_running else len(self.betas))):
            ...
            img_t = self.p_sample(denoise_fn=denoise_fn, data=img_t,t=t_, noise_fn=noise_fn, y=y,
                                  clip_denoised=clip_denoised, return_pred_xstart=False, topo_shape=topo_shape)
    ...
    def p_sample_loop_trajectory(self, denoise_fn, shape, device, y, freq,
        ...
        for t in reversed(range(0,total_steps)):
            ...
            img_t = self.p_sample(denoise_fn=denoise_fn, data=img_t, t=t_, noise_fn=noise_fn, y=y,
                                  ...
                                  return_pred_xstart=False, topo_shape=topo_shape)
    ...
    def _vb_terms_bpd(self, denoise_fn, data_start, data_t, t, y, clip_denoised: bool, return_pred_xstart: bool, topo_shape):
        true_mean, _, true_log_variance_clipped = self.q_posterior_mean_variance(x_start=data_start, x_t=data_t, t=t)
        model_mean, _, model_log_variance, pred_xstart = self.p_mean_variance(
            denoise_fn, data=data_t, t=t, y=y, clip_denoised=clip_denoised, return_pred_xstart=True, topo_shape=topo_shape)
    ...
    def p_losses(self, denoise_fn, data_start, t, noise=None, y=None, topo_shape=None):
        """
        ...
        if self.loss_type == 'mse':
            ...
            eps_recon = denoise_fn(data_t, t, y, topo_shape = topo_shape)
    ...
    def calc_bpd_loop(self, denoise_fn, x_start, y, clip_denoised=True, topo_shape=None):
        ...
        with torch.no_grad():
            ...
            for t in reversed(range(T)):
                ...
                new_vals_b, pred_xstart = self._vb_terms_bpd(
                    ...
                    y=y, clip_denoised=clip_denoised, return_pred_xstart=True, topo_shape=topo_shape)
...
class Model(nn.Module):
    def __init__(self, args, betas, loss_type: str, model_mean_type: str, model_var_type:str):
        ...
        if args.window_size > 0:
            ...
            if args.use_resampler:
                self.model = DiT3D_resampler_models[args.model_type](pretrained=args.use_pretrained, 
                                                    input_size=args.voxel_size, 
                                                    num_classes=args.num_classes
                                                    )
            else:
    ...
    def all_kl(self, x0, y, clip_denoised=True, topo_shape=None):
        total_bpd_b, vals_bt, prior_bpd_b, mse_bt =  self.diffusion.calc_bpd_loop(self._denoise, x0, y, clip_denoised, topo_shape=topo_shape)
    ...
    def _denoise(self, data, t, y, topo_shape=None):
        B, D,N= data.shape
        assert data.dtype == torch.float
        assert t.shape == torch.Size([B]) and t.dtype == torch.int64

        out = self.model(data, t, y, topo_shape=topo_shape)

        assert out.shape == torch.Size([B, D, N])
        return out

    def get_loss_iter(self, data, noises=None, y=None, topo_shape=None):
...
def get_dataset(dataroot, npoints,category):
    tr_dataset = ShapeNet15kPointClouds(root_dir=dataroot,
        ...
        random_subsample=True, patch_size=64)
    te_dataset = ShapeNet15kPointClouds(root_dir=dataroot,
        ...
        all_points_std=tr_dataset.all_points_std, patch_size=64
    )
    ...
    for epoch in range(start_epoch, opt.niter):
        ...
        for i, data in enumerate(dataloader):
            ...
            if opt.distribution_type == 'multi' or (opt.distribution_type is None and gpu is not None):
                ...
                y = y.cuda(gpu)
                topo_shape = topo_shape.cuda(gpu)

            elif opt.distribution_type == 'single':
                x = x.cuda()
                noises_batch = noises_batch.cuda()
                y = y.cuda()
                topo_shape = topo_shape.cuda()

            loss = model.get_loss_iter(x, noises_batch, y, topo_shape=topo_shape).mean()
        ...
        if (epoch + 1) % opt.diagIter == 0 and should_diag:
            ...
            kl_stats = model.all_kl(x, y, topo_shape=topo_shape)
            logger.info('      [{:>3d}/{:>3d}]    '
                         'x_range: [{:>10.4f}, {:>10.4f}],   '
                         'total_bpd_b: {:>10.4f},    '
                         'terms_bpd: {:>10.4f},  '
        ...
        if (epoch + 1) % opt.vizIter == 0 and should_diag:
            ...
            model.eval()
            with torch.no_grad():
                # generate topology shape
                vae = get_vae(opt.vae_path, x.device)
                topo_shape_eval = vae.sample(25, x.device) 
                topo_shape_list = vae.sample(1, x.device) 
                # topo_shape_eval = torch.randn(25, 2, 64, 64)
                # topo_shape_list = torch.randn(1, 2, 64, 64)
                x_gen_eval = model.gen_samples(new_x_chain(x, 25).shape, x.device, new_y_chain(y,25,opt.num_classes), clip_denoised=False, topo_shape=topo_shape_eval)
                x_gen_list = model.gen_sample_traj(new_x_chain(x, 1).shape, x.device, new_y_chain(y,1,opt.num_classes), freq=40, clip_denoised=False, topo_shape=topo_shape_list)
...
def parse_args():
    ...
    parser.add_argument('--use_resampler', default=False)
    ...
    # 64 vae2
    # parser.add_argument('--vae_path', help = "chair64", default='../../last.ckpt')
    # parser.add_argument('--vae_path', help = "airplane64", default=''../../last.ckpt')
    # parser.add_argument('--vae_path', help = "car64", default=''../../last.ckpt')
    # parser.add_argument('--vae_path', help = "all", default=''../../last.ckpt')
